...
**********************************************/
var EXTRA_LOGS = true;
var TO = ['clients.contevo@gmail.com'];
...
var SPREADSHEET_URL = 'https://docs.google.com/spreadsheets/d/18NiQPHHtv2ny-bre2waljq5vVIZ0J1RTLF7TVoSBfCg/edit#gid=0';
...
{
	var accountName = sheet.getRange(i, 1);
	var accountStatus = sheet.getRange(i, 3);
	
	var accountLastCompleted = getDateString(new Date(sheet.getRange(i, 5).getValue()),'MM-dd-yyyy');
	
	if(accountStatus.getValue() < '1' && accountLastCompleted != currDate)
	{
		accountSkipArray.push(accountName.getValue());
	}
	else
	{
		continue; 
...
function main() {
	...
	Logger.log('accounts to be processed: ' + accountSkipArray);
	...
	function processAccount() {
		...
		while (campaignIterator.hasNext()) {
			...
			//var currAccount = MccApp.select(account);

			// filter account to be processed
			...
			info('Customer ID: ' + currAccountId);
